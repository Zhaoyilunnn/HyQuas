...
std::pair<std::string, std::vector<qreal>> parse_gate(char buf[]) {
  ...
  int mult_or_div = 0; // 0 for mult, 1 for div
  while (i < l) {
    i++;
    std::string st;
    while (buf[i] != ',' && buf[i] != ')') {
    ...
    if (st[0] == 'p' && st[1] == 'i' && st[2] == '*') {
      ...
      param = pi;
      st = st.erase(0, 3);
    } else if (st[0] == '-' && st[1] == 'p' && st[2] == 'i' && st[3] == '*') {
      mult_or_div = 0;
      param = -pi;
      st = st.erase(0, 4);
    } else if (st[0] == '-' && st[1] == 'p' && st[2] == 'i' && st[3] == '/') {
...
std::unique_ptr<Circuit> parse_circuit(const std::string &filename) {
  ...
  while (fscanf(f, "%s", buffer) != EOF) {
    if (strcmp(buffer, "//") == 0 || strcmp(buffer, "OPENQASM") == 0 ||
      ...
      if (gate.first == "crx") {
        ...
        assert(qid.size() == 1);
        c->addGate(
            Gate::U3(qid[0], gate.second[0], gate.second[1], gate.second[2]));
        // printf("u3 %d %f %f %f\n", qid[0], gate.second[0], gate.second[1],
        // gate.second[2]);
